...
import com.incapp.ReCaptchaResponse;
...
public class UserController {
	...
	public String UserRegister(@ModelAttribute User u,ModelMap m, @RequestParam(name="g-recaptcha-response") String captchaResponse) {
		
		recaptchaSecret="?secret="+recaptchaSecret+"&response="+captchaResponse;
		ReCaptchaResponse reCaptchaResponse=restTemplate.exchange(recaptchaUrl+recaptchaSecret, HttpMethod.POST,null,ReCaptchaResponse.class).getBody();
		...
		if(reCaptchaResponse.isSuccess()) {
			...
			m.addAttribute("result", "Please Verify Captcha!");
			return "index";
		}
	}
